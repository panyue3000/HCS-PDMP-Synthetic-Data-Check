...
PROC IMPORT OUT= WORK.DEA0
            DATAFILE= "\\doh-smb\doh_shared\Projects\BNE\BNE_SCRIPT_DATA\Files\DEA33_FORHCS_0123 2.csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
	 GUESSINGROWS=60000;
...
data DEA0; set DEA0;
waiver_type1=scan(DW,-1,'-');
waiver_type=input(waiver_type1,best8.);
drop waiver_type1;
run;
...
run;
PROC IMPORT OUT= WORK.HCS_zips1   DATAFILE= "\\doh-smb\doh_shared\Projects\BNE\BNE_SCRIPT_DATA\Files\Zipcodes for 3 communities.xlsx" 
            DBMS=EXCEL REPLACE;
